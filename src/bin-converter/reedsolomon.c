...
static inline U8 GF256Mul( U8 u8N, U8 u8M )
{
  ...
  if( u16ResIdx >= 255 )
  {
    u16ResIdx -= 255;  // note: cau8Alpha[ 255 ] == cau8Alpha[ 0 ] == 1
  }
 ...
 * \return -
 * \note   Parity bytes are written to the last 4 byte.
 *********************************************************************/
void ReedSolomon_AddRS2824( U8* pu8TempFrame )
{
    ...
    for( u8Index = 0; u8Index < 24; u8Index++ )
    {
        ...
        {
            u8Temp[ u8Index + u8InnerIdx ] ^= GF256Mul( u8Multiplier, cau8RS2824Generator[ u8InnerIdx ] );
        }
    }
...
void ReedSolomon_AddRS3228( U8* pu8TempFrame )
{
    ...
    for( u8Index = 0; u8Index < 28; u8Index++ )
    {
        ...
        for( u8InnerIdx = 0; u8InnerIdx < 5; u8InnerIdx++ )
        {
            u8Temp[ u8Index + u8InnerIdx ] ^= GF256Mul( u8Multiplier, cau8RS3228Generator[ u8InnerIdx ] );
        }
    }
